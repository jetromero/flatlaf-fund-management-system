 ...
 * @author jetve
 */
public class CreateAttendee extends javax.swing.JPanel {
...
//        datePicker1.now();
//        datePicker1.setEditor(attendDate);
//        datePicker1.setEditorIcon(new FlatSVGIcon("jet/images/calendar.svg"));
        ...
        eventName.addItemListener(new ItemListener() {
            public void itemStateChanged(ItemEvent evt) {
                if (evt.getStateChange() == ItemEvent.SELECTED) {
                    String selectedEvent = eventName.getSelectedItem().toString();
                    if (!selectedEvent.equals("Select Event")) {
                        populateEventDate(selectedEvent);
                    }
                }
            }
        });
    ...
    private void initComponents() {
        ...
        attendYear.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Year Level", "1st Year", "2nd Year", "3rd Year", "4th Year" }));
        ...
        attendStatus.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Select Status", "Present", "Absent" }));
        ...
        attendName.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Search Attendee" }));
        attendName.setSelectedIndex(-1);
        ...
        layout.setHorizontalGroup(
            ...
            .addGroup(layout.createSequentialGroup()
                ...
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addComponent(attendStatus, javax.swing.GroupLayout.Alignment.TRAILING, 0, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    ...
                    .addComponent(eventName, 0, 300, Short.MAX_VALUE))
                .addContainerGap(100, Short.MAX_VALUE))
        );
...
//            attendName.removeAllItems();
    ...
    private void populateEventDate(String selectedEvent) {
        try {
            Connection conn = DatabaseConnection.getConnection();
            String sql = "SELECT start_t FROM events WHERE event_name = ?";
            PreparedStatement pst = conn.prepareStatement(sql);
            pst.setString(1, selectedEvent);
            ResultSet rs = pst.executeQuery();

            if (rs.next()) {
                String startDate = rs.getString("start_t");
                attendTimeIn.setText(startDate); // Set the date field
            }
            ...
            Notifications.getInstance().show(Notifications.Type.ERROR,
                "Error loading event date: " + e.getMessage());
        }
    }
    ...
    public boolean isAttendeeExists() {
        try {
            Connection conn = DatabaseConnection.getConnection();
            String sql = "SELECT COUNT(*) as count FROM attendees WHERE event = ? AND attendee_name = ?";
            PreparedStatement pst = conn.prepareStatement(sql);
